...
class ProfileController {
    ...
    updateUserRole = asyncWrapper(async (req, res, _next) => {
        const { user_data } = req.body;
        const { email } = req.payload?.currentUser;
        const updatedData = await User.findOneAndUpdate({ email }, { userRole: user_data }, { new: true });
    ...
    updateUserGoal = asyncWrapper(async (req, res, _next) => {
        const { user_data } = req.body;
        const { email } = req.payload?.currentUser;
        const updatedData = await User.findOneAndUpdate({ email }, { userGoal: user_data }, { new: true });
        if (updatedData && typeof updatedData === "object") {
            respondWith(res, apiStatus.success(), {
    ...
    updateUserPreference = asyncWrapper(async (req, res, _next) => {
        const { user_data } = req.body;
        const { email } = req.payload?.currentUser;
        const updatedData = await User.findOneAndUpdate({ email }, { userPreference: user_data }, { new: true });
        if (updatedData && typeof updatedData === "object") {
            respondWith(res, apiStatus.success(), {
    ...
    updateUserType = asyncWrapper(async (req, res, _next) => {
        const { user_type } = req.body;
        const { email } = req.payload?.currentUser;
        const updatedData = await User.findOneAndUpdate({ email }, { userType: user_type }, { new: true });
        if (updatedData && typeof updatedData === "object") {
            respondWith(res, apiStatus.success(), {
