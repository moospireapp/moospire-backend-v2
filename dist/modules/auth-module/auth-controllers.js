import { asyncWrapper, apiStatus, respondWith, serveCookieResponse, emailHandler, } from "../../core/index.js";
...
class AuthController {
    ...
    signupUser = asyncWrapper(async (req, res, next) => {
        ...
        const userPayload = await User.create({
            ...req.body,
            experienceLevel: req.body.email === env.SUPER_ADMIN_EMAIL
                ? UserRole.SUPERADMIN
                : UserRole.REGULAR,
        });
        ...
        const { user, token } = (await authService.generateUserPayload(userPayload));
    ...
    loginUser = asyncWrapper(async (req, res, next) => {
        const response_payload = await authService.validateUserSignIn(req.body.email, req.body.password, next);
        if (!response_payload)
            return;
        // Check if user data has been archived
        const userIsArchived = await authService.checkUserArchivedState(req.body.email, next);
        if (userIsArchived)
            return;
        const { user, token } = response_payload;
        serveCookieResponse(res, token);
    ...
    requestUserPassword = asyncWrapper(async (req, res, next) => {
        const userPayload = await authService.checkEmailExist(req.body.email, false);
        if (userPayload &&
            ...
            emailHandler.sendEmail(mailOptions, "request-password", templateOptions, next);
            respondWith(res, apiStatus.success(), {
    ...
    resetUserPassword = asyncWrapper(async (req, res, _next) => {
        ...
        if (typeof verifiedPayload !== "string" && verifiedPayload.email) {
            ...
            if (userPayload) {
                userPayload.password = await authService.hashPassword(req.body.password);
            ...
            respondWith(res, apiStatus.unauthorized("Reset token is either invalid or expired"));
        }
    });
    // Verifies user OTP for account verification
    verifyUserOTP = asyncWrapper(async (req, res, _next) => {
    ...
    resendUserOTP = asyncWrapper(async (req, res, next) => {
    ...
    // Handles user logout
    logoutUser = asyncWrapper(async (req, res, _next) => {
